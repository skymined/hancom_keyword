## 주식과 뉴스의 만남 주스 (날짜별 뉴스/기업 주가 연관 분석)

대상 기업 : 한글과 컴퓨터

### 1. 프로젝트 소개

주식회사에게 있어서 주식 가격이란 자금운용 및 회사가치라는 지표로써 굉장히 중요하게 다뤄진다. 우리는 마케팅 부분에 있어서 날짜별 뉴스를 분석하여 마케팅의 비용과 내용을 확인하여 더 효율적인 마케팅방법을 제시해보고자 한다.
...
- 날짜별 뉴스 발행수, 주가 및 거래량/가격 변동점 그래프

![그래프](./docs/images/checkpoint_graph_total.png)
...
![비율 계산그래프](./docs/images/result.png)
```
이 경우에도 사업기사(메타에 대한 사업)의 효과가 다른 기사들보다 비율적으로 높은 것을 확인할 수 있다.
```

따라서, 사내정보나 주식기사보다는 당시 메타에 대한 사업 기사가 효율적이므로 회사는 마케팅비용 대비 메타에 맞는 신규 사업 기사를 위주로 작성하는 것이 경제적일 것이다.
...
|결론|초기 가정에 대한 평가를 어떻게 할 것인지|효율성 기반의 단순 결론|첫 가정 자체가 단순한 가정이어서 결론 또한 비교적 단순했다, 실제 마케팅 집행에 대한 가격, 대상(뉴스사)의 데이터가 있었다면 더 정성적이고 확실한 결론을 도출할 수 있었을 것이다.|
...
|담당|수집 대상(기간)|코드|
|--|--|--|
|박요한|2022.01 ~ 2023.12|[news](./docs/selenium/yohan/stock_news.py) / [stock](./docs/selenium/yohan/get_stock.py)|
|김하늘|2018.01 ~ 2019.12||
|노주현|2020.01 ~ 2021.12||
...
|내용|산출물|코드|
|--|--|--|
|뉴스 전처리|[뉴스 카운팅.pkl](./docs/analysis/yohan/pickles/check_count.pkl) / [뉴스 데이터.pkl](./docs/analysis/yohan/pickles/news_data.pkl)|[ipynb](./docs/analysis/yohan/news_analysis.ipynb)|
|주가정보 전처리|[주가 데이터.pkl](./docs/analysis/yohan/pickles/stock_data.pkl) / [체크포인트 날짜.pkl](./docs/analysis/yohan/pickles/date_dataframe.pkl)|[ipynb](./docs/analysis/yohan/sotck_analysis.ipynb)|
|토픽 분석||[ipynb](./docs/analysis/yohan/total_analysis.ipynb)|


<img src="./docs/images/stock_line.png" alt="자체제작 그래프" width="400"/> <img src="./docs/images/stock_from_naver.png" alt="네이버 그래프" width="400"/>
```
네이버 주식 그래프와 비교했을 때, 큰 오류는 보이지 않는다.
```
<img src="./docs/images/checkpoint_count.png" alt="체크포인트 카운트" width="400"/> <img src="./docs/images/stock_changed.png" alt="체크포인트 비율" width="400"/>
```
...
![체크포인트 하이라이트 그래프](./docs/images/checkpoint_graph.png)
```
붉은 점이 눈여겨볼만한 곳 - 가격과 트레이딩이 모두 외부요인이 작용했다고 여겨지는 날짜
```

![체크포인트 뉴스 카운트 그래프](./docs/images/check_count.png)
```
...
![5개 이상 전일비값 top 10](./docs/images/daily_change_count.png)
...
1. 실적에 대한 급등주 or 테마주로 인한 주식 기사효과
    -> 이 경우 직접적으로 마케팅으로 집행하기 어렵고, 비용도 크게 발생하지 않았을 것
2. 사업 진행에 대한 기대 기사효과
