...
|분석|전일비와 뉴스 갯수, 토픽의 상관관계에 대한 연관성이 약함|뉴스 갯수를 기준으로 그룹화|전일비에 대한 계산이 투자자,기관,외국인에 대한 데이터가 있었다면 더 정확했을 것같다.|
...
|결론|효율성 도출|이상치를 제외한 가격에 대해 뉴스개수로 평균화|날짜별로 세분화를 시켜 표현했다면 더 좋았을 것, 세분화된 내용의 수치화를 따로 진행하면 더 좋았을 것|
